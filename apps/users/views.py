...
from users.serializers import AddressListModelSerializer, UserUpdateSerializer, \
    RegisterUserModelSerializer, \
    LoginUserModelSerializer, \
    UserWishlist


@extend_schema(tags=['user.json'])
...
class LoginAPIView(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        serializer = self.get_serializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        user = serializer.validated_data['user.json']
...
class AddressListCreateAPIView(ListCreateAPIView):
    ...
    def get_queryset(self):
        return super().get_queryset().filter(user=self.request.user).order_by('address')


@extend_schema(tags=['shops'])
class AddressDestroyUpdateAPIView(mixins.UpdateModelMixin, mixins.DestroyModelMixin,
                                  GenericAPIView):
    queryset = Address.objects.all()
    serializer_class = AddressListModelSerializer
    permission_classes = IsAuthenticated,
    ...
    def delete(self, request, *args, **kwargs):
        ...
        _user: User = request.user
        if self._can_delete and instance.id not in (
        _user.billing_address_id, _user.shipping_address_id):
